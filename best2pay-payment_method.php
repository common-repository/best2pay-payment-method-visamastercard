...
/*
 * Plugin Name: Best2Pay payment module
 * Plugin URI: https://best2pay.net/
 * Description: Receive payments via Visa / Mastercard / MIR easily with Best2Pay bank cards processing
 * Version: 3.0.0
 * Author: Best2Pay
 * Tested up to: 6.6.1
 * License: GPL3
...
function init_woocommerce_best2pay() {
    ...
    class woocommerce_best2pay extends WC_Payment_Gateway {
        const PLUGIN_URL = 'https://best2pay.net';
        ...
        public function __construct() {
            ...
            $this->description = $this->settings['description'] ? $this->settings['description'] : sprintf(__('Payments with bank cards via the <a href="%s" target="_blank">Best2Pay</a> payment system.', 'best2pay-payment_method'), self::PLUGIN_URL);
            ...
            switch($this->payment_method) {
                ...
                case 'authorizeWithInstallment':
                    $this->title = __('Pay for your order in installments', 'best2pay-payment_method');
            ...
            add_action('woocommerce_api_best2pay_notify', [$this, 'callback_notify']);
            add_action('woocommerce_api_best2pay_complete_action', [$this, 'process_complete']);
            ...
            add_action("woocommerce_order_status_changed", [$this, 'wc_best2pay_order_payment_change'], 10, 4);
        }
        ...
        public function init_form_fields() {
            ...
            $this->form_fields = [
                ...
                'title' => [
                    'title' => __('Title', 'best2pay-payment_method'),
                    'type' => 'text',
                    'description' => __('Custom title for payment type', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'placeholder' => $this->method_title,
                    'default' => ''
                ],
                'description' => [
                    'title' => __('Description', 'best2pay-payment_method'),
                    'type' => 'textarea',
                    'description' => __('Custom description for payment type', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'css' => 'width: 400px;',
                    'default' => ''
                ],
                'logo' => [
                    'title' => __('Logo', 'best2pay-payment_method'),
                    'type' => 'image',
                    'description' => __('Custom logo for payment type', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'default' => ''
                ],
                'sector' => [
                ...
                'testmode' => [
                    ...
                    'type' => 'select',
                    'options' => [
                        '1' => __('Test mode - real payments will not be processed', 'best2pay-payment_method'),
                        '0' => __('Production mode - payments will be processed', 'best2pay-payment_method')
                    ],
                ...
                'payment_method' => [
                    'title' => __('Payment method', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => [
                        'purchase' => __('Standard acquiring (one-stage payment)', 'best2pay-payment_method'),
                        'authorize' => __('Standard acquiring (two-stage payment)', 'best2pay-payment_method') . ' *',
                        'purchaseWithInstallment' => __('Plait (one-stage payment)', 'best2pay-payment_method'),
                        'authorizeWithInstallment' => __('Plait (two-stage payment)', 'best2pay-payment_method') . ' *',
                        'purchaseSBP' => __('Fast Payment System', 'best2pay-payment_method'),
                        'loan' => __('Loan', 'best2pay-payment_method')
                    ],
                    'description' => '* ' . __('Payment occurs after confirmation by the manager in the personal account', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'default' => 'purchase'
                ],
                'tax' => [
                    'title' => __('TAX', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => [
                        1 => __('VAT rate 20%', 'best2pay-payment_method'),
                        2 => __('VAT rate 10%', 'best2pay-payment_method'),
                        3 => __('VAT rate calc. 20/120', 'best2pay-payment_method'),
                        4 => __('VAT rate calc. 10/110', 'best2pay-payment_method'),
                        5 => __('VAT rate 0%', 'best2pay-payment_method'),
                        6 => __('Not subject to VAT', 'best2pay-payment_method'),
                    ],
                    'default' => '6'
                ],
                'notify_url' => [
                    'title' => __('Notify URL', 'best2pay-payment_method'),
                    'type' => 'text',
                    'description' => __('Report this URL to Best2Pay technical support to receive payment notifications', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'custom_attributes' => [
                        'readonly' => 'readonly',
                    ],
                ...
                'hash_algo' => [
                    'title' => __('Data encryption algorithm', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => [
                        'md5' => 'MD5',
                        'sha256' => 'SHA256',
                    ],
                    'description' => __('Must match your sector encryption settings in your personal account', 'best2pay-payment_method'),
                    'desc_tip' => true,
                    'default' => 'md5'
                ],
                'custom_statuses_header' => [
                    'title' => __('Custom statuses for orders', 'best2pay-payment_method'),
                    'type' => 'title'
                ],
                'registered_status' => [
                    'title' => __('Order registered', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-pending'
                ],
                'authorized_status' => [
                    'title' => __('Order authorized', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-on-hold'
                ],
                'loan_status' => [
                    'title' => __('Loan agreement approved but not signed', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-processing'
                ],
                'completed_status' => [
                    'title' => __('Order successfully paid', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-completed'
                ],
                'canceled_status' => [
                    'title' => __('Order canceled', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-refunded'
                ],
                'notify_customer_enabled' => [
                    'title' => __('Issuing an invoice for payment', 'best2pay-payment_method'),
                    'type' => 'checkbox',
                    'label' => __('Enable issuing an invoice for payment by email of the payer', 'best2pay-payment_method'),
                    'default' => 'no'
                ],
                'agreement_status' => [
                    'title' => __('Order agreement', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-on-hold'
                ],
                'payment_expected_status' => [
                    'title' => __('Payment expected', 'best2pay-payment_method'),
                    'type' => 'select',
                    'options' => $wc_statuses,
                    'default' => 'wc-pending'
                ]
            ...
            wp_enqueue_script('admin-best2pay-script', plugins_url('assets/js/admin.js', __FILE__));
        }
        ...
        public function payment_fields() {
            if($this->payment_method === 'purchaseWithInstallment' || $this->payment_method === 'authorizeWithInstallment')
                echo '<iframe style="width:100%;height:180px;border:none;min-width: 440px;margin-left: -50px;" src="' . plugins_url('svkb_widget.php', __FILE__) . '?amount=' . print_r((isset(WC()->cart->cart_contents_total)) ? WC()->cart->cart_contents_total : '', true) . '"></iframe>';
        }

        public function admin_options() {?>
            <h3><?php _e('Best2Pay', 'best2pay-payment_method');?></h3>
            <p><?php echo sprintf(__('Payments with bank cards via the <a href="%s" target="_blank">Best2Pay</a> payment system.', 'best2pay-payment_method'), self::PLUGIN_URL);?></p>
                ...
                <td class="forminp">
                    ...
                    <?php if(!empty($this->logo)){
                        ...
                        <label for="<?php echo esc_attr($this->remove_logo_field);?>">
                            <input class="" type="checkbox" name="<?php echo esc_attr($this->remove_logo_field);?>" id="<?php echo esc_attr($this->remove_logo_field);?>" style="" value="1"> <?php echo wp_kses_post( __('Remove logo', 'best2pay-payment_method'));?>
                        </label>
                    <?php } else { ?>
                        <img src="<?php echo plugins_url('assets/img/best2pay.png', plugin_basename(__FILE__));?>" alt="" width="100" style="margin: 10px">
                    <?php } ?>
                </td>
            </tr>
        ...
        function wc_order_item_add_complete_button($order) {
            if($order->get_payment_method() === $this->id && get_post_meta($order->get_id(), 'best2pay_order_state', true) === 'AUTHORIZED') {?>
                <script src="<?php echo plugins_url('assets/js/scripts.js', plugin_basename(__FILE__));?>"></script>
                <input type="hidden" id="nonce_best2pay_complete" value="<?php echo wp_create_nonce('best2pay_complete_action' . $order->get_id());?>">
                <button type="button" id="button_best2pay_complete" class="button custom-items"><?php echo __('Complete payment', 'best2pay-payment_method');?></button>
            <?php }
        }
        ...
        public function process_payment($order_id) {
            ...
            if($this->notify_customer_enabled && $wc_order->get_status() !== $this->agreement_status && !get_post_meta($order_id, 'best2pay_order_moderated', true)) {
                $wc_order->add_order_note(__('Order created successfully', 'best2pay-payment_method'));
                $wc_order->update_status($this->agreement_status);

                update_post_meta($wc_order->get_id(), 'best2pay_order_moderated', 'yes');
            ...
            $register_order_id = get_post_meta($order_id, 'best2pay_order_id', true);
        ...
        public function wc_best2pay_order_payment_change($order_id, $status_from, $status_to, $order) {
            if($this->notify_customer_enabled && $order->payment_method === 'best2pay' && ($status_from === 'on-hold' && $status_to === 'pending'))
        ...
        public function process_payment_registration($order): int {
            ...
            try {
                ...
                if (!$b2p_order_id)
                    throw new Exception(__('Failed to get Best2pay order ID', 'best2pay-payment_method'));
            } catch (Exception $e) {
                wc_add_notice($e->getMessage(), 'error');
            }

            $order->update_status($this->{strtolower($response->getState()) . '_status'});
            update_post_meta($order->get_id(), 'best2pay_order_id', $b2p_order_id);
            update_post_meta($order->get_id(), 'best2pay_order_state', $response->getState());
            $order->add_order_note(__('Order registered successfully', 'best2pay-payment_method') . " (ID: $b2p_order_id)");
            ...
            try {
                if ($isNotifyRequest = $this->isNotifyRequest()) {
                    ...
                    if(isset($_REQUEST['error'])) {
                        $message = __('Failed to pay for the order', 'best2pay-payment_method') . ":\n" . $_REQUEST['error'];
                    ...
                    if(!$pc_ref_id)
                        throw new Exception(__('Undefined order ID', 'best2pay-payment_method'));

                    $pc_order_id = (int)get_post_meta($pc_ref_id, 'best2pay_order_id', true);
                    if($ct_order_id !== $pc_order_id)
                        throw new Exception(__('Request data is not valid', 'best2pay-payment_method'));
                }
                ...
                if(!$wc_order)
                    throw new Exception(__('Failed to get order information', 'best2pay-payment_method'));

                $wc_order->update_meta_data('best2pay_order_state', $ct_order->getState());

                $paid = false;

                if($ct_order->getState() !== get_post_meta($wc_order->get_id(), 'best2pay_order_state', true)) {
                    if($ct_order instanceof CreditOrder) {
                        $wc_order->update_status($ct_order->isPaid() ? $this->completed_status : $this->loan_status);
                        $wc_order->add_order_note($ct_order->isPaid() ? __('The loan agreement was successfully completed and signed', 'best2pay-payment_method') : __('The loan agreement was successfully completed, but not signed', 'best2pay-payment_method'));

                        $paid = true;
                    } else {
                        $wc_order->update_status($this->{strtolower($ct_order->getState()) . '_status'});
                        $wc_order->add_order_note(__('Payment ' . strtolower($ct_order->getState()) . ' successfully', 'best2pay-payment_method'));
                ...
                if ($isNotifyRequest) {
                    ...
                    if($paid) {
                        ...
                        wc_add_notice(nl2br(__('Failed to pay incorrect card', 'best2pay-payment_method')), 'error');
                        wp_redirect($checkout_url);
                    }
                }
            } catch (\throwable $e) {
        ...
        public function process_complete() {
            try {
                ...
                if (!wp_verify_nonce($_REQUEST['best2pay_nonce_value'], 'best2pay_complete_action' . $order_id))
                    throw new Exception(__('Operation failed. Please refresh the page', 'best2pay-payment_method'));

                $wc_order = wc_get_order($order_id);
                if(!$wc_order)
                    throw new Exception(__('Failed to get order information', 'best2pay-payment_method'));

                $pc_order_id = get_post_meta($wc_order->get_id(), 'best2pay_order_id', true);
                if(!$pc_order_id)
                    throw new Exception(__('Failed to get Best2pay order ID', 'best2pay-payment_method'));
                ...
                if(!$complete_result)
                    throw new Exception(__('Unable to debit funds', 'best2pay-payment_method'));

                if($ct_order instanceof CreditOrder) {
                    $success_message = __('The loan agreement was successfully completed and signed', 'best2pay-payment_method');
                } else {
                    $wc_order->update_status($this->completed_status);
                    $success_message = __('Funds for the order have been successfully debited', 'best2pay-payment_method');
                }

                $wc_order->update_meta_data('best2pay_order_state', 'COMPLETED');
        ...
        public function process_refund($order_id, $amount = null, $reason = '') {
            try {
                ...
                if($amount !== $wc_order->get_total())
                    return new WP_Error('error', __('Error, please enter the refund amount. Refunds must be made for the full amount of the order', 'best2pay-payment_method'));
                ...
                if($reverse_result instanceof Error)
                    return new WP_Error('error', __('Unable to issue a refund on a credit order', 'best2pay-payment_method'));
            } catch (Exception $e) {
                echo json_encode([
                    'success' => false,
                    'message' => nl2br($e->getMessage())
            ...
            $wc_order->update_meta_data('best2pay_order_state', 'CANCELED');
            $comment = __('Payment canceled successfully', 'best2pay-payment_method');
    ...
    add_filter('plugin_action_links_' . plugin_basename( __FILE__ ),
        function($links) {
            ...
            array_unshift($links, sprintf('<a href="%1$s">%2$s</a>', admin_url('admin.php?page=wc-settings&tab=checkout&section=best2pay'), __('Settings', 'best2pay-payment_method')));
